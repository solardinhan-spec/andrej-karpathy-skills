...
export default function QuickModal({ onClose, onSave, names }) {
  ...
  const key = (k) => { if (err) setErr(''); setQ((s) => {
    let a = s.amount
    if (k === '⌫') a = a.slice(0, -1)
  ...
  const save = () => {
    ...
    if (!amt) { setErr('금액을 입력해 주세요'); return }
  ...
  return (
    <>
      ...
      <div style={sheet}>
        ...
        {err && <div style={{ textAlign: 'center', fontSize: 13, fontWeight: 600, color: '#F04452', marginTop: 12 }}>{err}</div>}
        <div className="press" onClick={save} style={{ marginTop: err ? 8 : 16, textAlign: 'center', padding: 16, borderRadius: 15, fontSize: 16, fontWeight: 700, color: '#fff', background: '#3182F6', boxShadow: '0 6px 16px rgba(49,130,246,.32)' }}>저장하기</div>
      </div>
    </>
  )
}
