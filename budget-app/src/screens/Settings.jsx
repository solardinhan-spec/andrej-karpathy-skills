    ...
    <div className="card" style={{ marginBottom: 12, overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, color: '#9CA3AF', display: 'inline-block', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▼</span>
        </div>
      </button>
      {open && (
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 54px', gap: 6, marginBottom: 8, alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        <button onClick={onDelete} style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 6, cursor: 'pointer', fontSize: 10, color: '#EF4444', padding: '2px 4px', fontFamily: 'inherit' }}>삭제</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="screen" style={{ padding: '0 14px 80px' }}>
      <div style={{ padding: '24px 0 16px' }}>
        <p style={{ fontSize: 12, color: '#9CA3AF', marginBottom: 4 }}>우리 가계부</p>
        <h1 style={{ fontSize: 22, fontWeight: 800 }}>⚙️ 설정</h1>
        {saved && <p style={{ fontSize: 12, color: '#16A34A', marginTop: 6, fontWeight: 600 }}>✅ 자동 저장됨</p>}
      </div>

      {/* 예산 요약 */}
      <div style={{ background: '#111111', borderRadius: 14, padding: '14px 16px', marginBottom: 14, color: 'white' }}>
        <p style={{ fontSize: 11, color: '#6B7280', marginBottom: 8 }}>현재 예산 구조</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[
            { label: '총수입', value: total, color: '#4ADE80' },
            { label: '고정지출', value: fixed, color: '#C0622A' },
            { label: '저축+투자', value: settings.savings + settings.investment, color: '#60A5FA' },
            { label: '기타예산', value: Math.max(variable, 0), color: variable < 0 ? '#F87171' : '#FCD34D' },
          ].map(item => (
            <div key={item.label}>
        ...
        {variable < 0 && <p style={{ fontSize: 11, color: '#F87171', marginTop: 8 }}>⚠️ 지출 합계가 수입을 초과해요!</p>}
      </div>
      ...
      <AccordionSection title="💰 수입" total={total}>
        <div style={{ marginTop: 14 }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 5 }}>이현 월급</label>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 5 }}>혜원 수입</label>
      ...
      <AccordionSection title="📋 고정지출 - 이현" total={ihyeonFixed}>
        <div style={{ marginTop: 14 }}>
          ...
          <button onClick={() => addFixed('이현')} style={{ width: '100%', padding: 9, borderRadius: 10, border: '2px dashed #E0D5C8', background: 'white', color: '#9CA3AF', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', marginTop: 4 }}>
            + 항목 추가
          </button>
        </div>
      </AccordionSection>
      ...
      <AccordionSection title="📋 고정지출 - 혜원" total={hyewonFixed}>
        <div style={{ marginTop: 14 }}>
          ...
          <button onClick={() => addFixed('혜원')} style={{ width: '100%', padding: 9, borderRadius: 10, border: '2px dashed #E0D5C8', background: 'white', color: '#9CA3AF', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', marginTop: 4 }}>
            + 항목 추가
          </button>
        </div>
      </AccordionSection>
      ...
      <AccordionSection title="💚 저축·투자 목표" total={settings.savings + settings.investment}>
        <div style={{ marginTop: 14 }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 5 }}>저축 목표</label>
            <input type="number" value={settings.savings} onChange={e => set(s => ({ ...s, savings: Number(e.target.value) || 0 }))} style={inputStyle} />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 5 }}>투자 목표</label>
      ...
      <AccordionSection title="💵 용돈·비상예비금" total={settings.allowance.ihyeon + settings.allowance.hyewon + settings.reserve}>
        <div style={{ marginTop: 14 }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 5 }}>이현 용돈</label>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 5 }}>혜원 용돈</label>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', display: 'block', marginBottom: 5 }}>비상예비금</label>
      ...
      <AccordionSection title="🌡️ 기타지출 항목" total={catTotal}>
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 54px', gap: 6, marginBottom: 6 }}>
            <span style={{ fontSize: 11, color: '#9CA3AF', fontWeight: 600 }}>항목명</span>
            <span style={{ fontSize: 11, color: '#9CA3AF', fontWeight: 600, textAlign: 'right' }}>예산(원)</span>
            <span style={{ fontSize: 11, color: '#9CA3AF', fontWeight: 600, textAlign: 'center' }}>담당</span>
          </div>
          {catBudget.map(b => (
            <EditRow key={b.id}
          ...
          <button onClick={addCat} style={{ width: '100%', padding: 9, borderRadius: 10, border: '2px dashed #E0D5C8', background: 'white', color: '#9CA3AF', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', marginTop: 4 }}>
            + 항목 추가
          </button>
        </div>
      </AccordionSection>
