...
const CONTENT_CARDS = [
  {
    id: 'c1', category: '절약', color: '#5F8A6E', bg: '#EFF7F2',
  ...
  {
    id: 'c4', category: '재테크', color: '#C8960A', bg: '#FEF9EC',
  ...
  {
    id: 'c6', category: '재테크', color: '#D94040', bg: '#FEF0F0',
    emoji: '💳', title: '체크카드 vs 신용카드',
    desc: '소비 통제엔 체크카드, 혜택은 신용카드. 월 지출의 30%는 캐시백 신용카드로 혜택 극대화',
    tag: '카드',
  },
  {
    id: 'c7', category: '절약', color: '#5F8A6E', bg: '#EFF7F2',
    emoji: '🚗', title: '교통비 절약',
  ...
  {
    id: 'c10', category: '재테크', color: '#C8960A', bg: '#FEF9EC',
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={{ position: 'relative', width: 80, height: 100, margin: '0 auto 8px' }}>
        ...
        <div style={{ position: 'absolute', bottom: 8, left: '50%', transform: 'translateX(-50%)', fontSize: 10, fontWeight: 700, color: pct > 0.4 ? 'white' : '#9A8A7A' }}>
          {Math.round(pct * 100)}%
        </div>
      </div>
      <p style={{ fontSize: 14, fontWeight: 700, marginBottom: 2 }}>{jar.label}</p>
      <p style={{ fontSize: 11, color: '#9A8A7A' }}>{jar.current.toLocaleString('ko-KR')} / {jar.target.toLocaleString('ko-KR')}</p>
    </div>
  );
}
...
function EggDisplay({ streak }) {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 13, color: '#9A8A7A' }}>{streak}개월 연속 저축 달성</p>
      <p style={{ fontSize: 11, color: '#B0A090', marginTop: 6 }}>👆 탭하면 흔들려요</p>
    </div>
  );
}
  ...
  return (
    <div className="screen" style={{ padding: '0 16px 80px' }}>
      <div style={{ padding: '24px 0 20px' }}>
        <p style={{ fontSize: 12, color: '#9A8A7A', marginBottom: 4 }}>함께 성장하기</p>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginTop: 16 }}>
          {savingsLog.slice(-6).map(log => (
            <div key={log.month} title={log.month}
              style={{ width: 32, height: 32, borderRadius: '50%', background: log.hit ? '#C0622A' : '#F0EBE3', border: `2px solid ${log.hit ? '#C0622A' : '#E0D5C8'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
              {log.hit ? '✓' : '·'}
            </div>
          ))}
        </div>
      ...
      <div className="card" style={{ padding: '20px 20px 24px', marginBottom: 14 }}>
        ...
        <div style={{ marginTop: 16, background: '#FAF8F3', borderRadius: 12, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <p style={{ fontSize: 11, color: '#9A8A7A', marginBottom: 3 }}>💰 {monthsElapsed}개월 누적 (연 4% 복리)</p>
            <p style={{ fontSize: 18, fontWeight: 800, color: '#5F8A6E' }}>{accumulated.toLocaleString('ko-KR')}원</p>
          </div>
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 10, color: '#9A8A7A' }}>이자 수익</p>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#5F8A6E' }}>+{(accumulated - monthlyTarget * monthsElapsed).toLocaleString('ko-KR')}원</p>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          {CATEGORIES_FILTER.map(cat => (
            <button key={cat} onClick={() => setFilter(cat)}
              style={{ padding: '6px 14px', borderRadius: 99, border: `2px solid ${filter === cat ? '#C0622A' : '#E0D5C8'}`, background: filter === cat ? '#C0622A' : 'white', color: filter === cat ? 'white' : '#7A6A5A', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              {cat}
            </button>
          ))}
        </div>
            ...
            <div key={card.id} className="card" style={{ padding: 16, background: card.bg, boxShadow: 'none', border: `1px solid ${card.color}20` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: 14, fontWeight: 700, marginBottom: 4, color: '#1A1410' }}>{card.title}</p>
